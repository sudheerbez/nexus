...
export default function HomePage() {
    ...
    const stage = user?.current_stage || 'Dependent';
    const artifactCount = user?.artifacts?.length || 0;
    const verifiedCount = user?.artifacts?.filter(a => a.status === 'verified').length || 0;
    const stageIndex = ['Dependent', 'Interested', 'Involved', 'Self-Directed'].indexOf(stage);
    const progressPct = Math.round(((stageIndex + 1) / 4) * 100);

    const stats = [
        { label: "Current Stage", value: stage, sub: `Stage ${stageIndex + 1} of 4`, icon: Target, iconBg: "bg-blue-500/20", iconColor: "text-blue-400" },
        ...
        { label: "Career Interest", value: user?.career_interest || "Not set", sub: user?.major || "", icon: Users, iconBg: "bg-pink-500/20", iconColor: "text-pink-400" },
        { label: "Overall Progress", value: `${progressPct}%`, sub: "to self-directed", icon: TrendingUp, iconBg: "bg-purple-500/20", iconColor: "text-purple-400" },
    ];
            ...
            <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto">
                ...
                <div className="glass-card p-8 bg-gradient-to-r from-blue-900/50 to-purple-900/50 border-l-4 border-l-blue-400">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        <button onClick={() => navigate('/discovery')} className="glass-button bg-blue-500/20 hover:bg-blue-500/40 border-blue-400/40 text-blue-100 shrink-0">
                            <Sparkles size={16} /> Go to Discovery <ArrowRight size={16} />
                        </button>
                    </div>
                </div>
                    ...
                    {stats.map((stat, i) => (
                        <div key={i} className="glass-card p-5 flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                                ...
                                <div className={`p-1.5 rounded-lg ${stat.iconBg} ${stat.iconColor}`}><stat.icon size={16} /></div>
                            </div>
                            <p className="text-2xl font-bold text-white capitalize truncate">{stat.value}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <button onClick={() => navigate('/discovery')} className="glass-card p-6 flex flex-col items-start gap-3 text-left group">
                        <div className="p-3 rounded-xl bg-blue-500/20 text-blue-400 group-hover:bg-blue-500/30 transition-colors"><Compass size={24} /></div>
                    ...
                    <button onClick={() => navigate('/mentorship')} className="glass-card p-6 flex flex-col items-start gap-3 text-left group">
                        <div className="p-3 rounded-xl bg-purple-500/20 text-purple-400 group-hover:bg-purple-500/30 transition-colors"><BookOpen size={24} /></div>
                    ...
                    <button onClick={() => navigate('/settings')} className="glass-card p-6 flex flex-col items-start gap-3 text-left group">
                        <div className="p-3 rounded-xl bg-slate-500/20 text-slate-400 group-hover:bg-slate-500/30 transition-colors"><Users size={24} /></div>
                ...
                {artifactCount > 0 && (
                    <div className="glass-card p-6">
                        ...
                        <div className="space-y-3">
                            {user.artifacts.slice(-5).reverse().map((art, i) => (
                                <div key={art.id || i} className="flex items-center gap-4 p-3 bg-black/20 rounded-xl border border-white/5">
                                    <div className={`p-2 rounded-lg ${art.status === 'verified' ? 'bg-green-500/20 text-green-400' : art.status === 'rejected' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                                        <FileCode size={18} />
                                    </div>
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded-full shrink-0 ${art.status === 'verified' ? 'bg-green-500/20 text-green-300' : art.status === 'rejected' ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
                                        {art.status}
                                    </span>
                                </div>
                            ))}
