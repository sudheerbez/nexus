...
import { Home, Compass, User, Settings, Bell, Search, X } from 'lucide-react';
...
export default function DashboardLayout({ children, userData }) {
    ...
    const [notifications, setNotifications] = useState([
        { id: 1, text: "Mentor Agent reviewed your latest artifact", time: "2 min ago", unread: true },
        { id: 2, text: "New accountability partner match available", time: "1 hour ago", unread: true },
        { id: 3, text: "You advanced to the Interested stage!", time: "Yesterday", unread: false },
    ]);
        ...
        <div className="min-h-screen flex text-slate-100 font-sans">
            ...
            <aside className="w-20 lg:w-64 glass-panel m-4 flex flex-col items-center lg:items-start p-4 transition-all duration-300 z-20 overflow-visible">
                <div className="flex items-center gap-3 w-full mb-10 px-2 lg:px-4 relative z-10">
                    <div
                        className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-400 to-purple-500 shadow-lg shadow-blue-500/20 flex items-center justify-center font-bold cursor-pointer"
                        onClick={() => navigate('/')}
                    >N</div>
                    <span
                        className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400 hidden lg:block tracking-wide cursor-pointer"
                        onClick={() => navigate('/')}
                    >Nexus</span>
                </div>
                    ...
                    {navItems.map((item, i) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-4 w-full p-3 rounded-xl transition-all cursor-pointer ${active ? 'bg-white/10 shadow-lg border border-white/10 text-blue-300' : 'hover:bg-white/5 text-slate-400 hover:text-white border border-transparent'}`}
                            >
                    ...
                    <div className="flex items-center gap-4 ml-auto relative z-10">
                        ...
                        <div className="relative" ref={notifRef}>
                            ...
                            >
                                ...
                                {unreadCount > 0 && (
                                    <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-pink-500 rounded-full text-[10px] flex items-center justify-center font-bold animate-pulse">
                                        {unreadCount}
                                    </span>
                                )}
                            </button>
                                ...
                                <div className="absolute right-0 top-12 w-80 bg-slate-800/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl z-[100]" style={{ overflow: 'hidden' }}>
                                    <div className="flex items-center justify-between p-4 border-b border-white/10">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={() => setShowNotifications(false)} className="text-slate-400 hover:text-white transition-colors cursor-pointer">
                                                <X size={16} />
                                            </button>
                                        </div>
                                    </div>
                                    <div className="max-h-64 overflow-y-auto">
                                        {notifications.map((notif) => (
                                            <div
                                                key={notif.id}
                                                className={`p-4 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer ${notif.unread ? 'bg-blue-500/10' : ''}`}
                                                onClick={() => {
                                                    setNotifications(prev => prev.map(n => n.id === notif.id ? { ...n, unread: false } : n));
                                                }}
                                            >
                                                <div className="flex items-start gap-2">
                                                    {notif.unread && <span className="w-2 h-2 mt-1.5 bg-blue-400 rounded-full shrink-0"></span>}
                                                    <div>
                        ...
                        <div className="flex items-center gap-3 pl-4 border-l border-white/10">
                            <div className="hidden md:flex flex-col items-end">
                                <span className="text-sm font-medium">{userData?.name || 'Student'}</span>
                                <span className="text-xs text-blue-300 capitalize">{userData?.current_stage || 'Interested'} Stage</span>
                            </div>
